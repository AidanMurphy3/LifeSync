...
const getAllGroups = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Failed to retrieve groups.", error: e.message });
  }
};
...
const getGroupById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error retrieving group.", error: e.message });
  }
};
...
const addGroup = async (req, res) => {
  ...
  if (!name || !description || !owner) {
    return res.status(400).json({
      message:
        "Missing required fields: name, description, and owner (User ID).",
    });
  }
...
const deleteGroup = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Group deleted successfully", data: group });
  } catch (error) {
